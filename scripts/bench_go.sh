...
do
  ...
  do
    ...
    for j in {1,2,3,4,5}; do
      RUN=$($CMD bench -write-mode $1 -read-mode $2 -count $3 -batch-size $BATCH -profile-mode n 2>&1 | awk '{sub(/\(/, "", $5);print $5}' | tr '\n' ',' | sed 's/,,//')
      echo $1,$2,$3,$BATCH,$RUN | awk -F',' '{print "go," $1 "," $2 "," $3 "," $4 "," $5 "," $6}'
    done
  done
done